...
use octacity_server_application::{AgentEnrollmentSecretKey, CacheCredentialKey, JobSpecToolchainPolicy};
...
impl RuntimeDependencies {
  pub(crate) async fn from_config(config: &ServerConfig) -> Result<Self, ReadinessSetupError> {
    ...
    let cache_credential_key = read_cache_credential_key(&config.cache().credential_key_file).await?;
...
async fn read_agent_enrollment_key(path: &Path) -> Result<AgentEnrollmentSecretKey, ReadinessSetupError> {
  read_fixed_key("Agent enrollment derivation key", path, AgentEnrollmentSecretKey::new).await
}

async fn read_cache_credential_key(path: &Path) -> Result<CacheCredentialKey, ReadinessSetupError> {
  read_fixed_key("cache credential derivation key", path, CacheCredentialKey::new).await
}

async fn read_fixed_key<const N: usize, T>(
  purpose: &'static str,
  path: &Path,
  constructor: impl FnOnce([u8; N]) -> T,
) -> Result<T, ReadinessSetupError> {
  let encoded = read_credential(purpose, path).await?;
