...
pub(crate) struct ObjectStorageConfig {
  ...
  #[serde(default = "default_upload_capability_milliseconds")]
  upload_capability_lifetime_milliseconds: u64,
  #[serde(default = "default_download_capability_milliseconds")]
  download_capability_lifetime_milliseconds: u64,
}
...
impl ObjectStorageConfig {
  pub(crate) fn validate(&self) -> Result<(), String> {
    ...
    for (name, value) in [
      (
        "object_storage.upload_capability_lifetime_milliseconds",
        self.upload_capability_lifetime_milliseconds,
      ),
      (
        "object_storage.download_capability_lifetime_milliseconds",
        self.download_capability_lifetime_milliseconds,
      ),
    ] {
      if value == 0 || value > MAX_TRANSFER_CAPABILITY_MILLISECONDS {
        return Err(format!(
          "{name} must be between 1 and {MAX_TRANSFER_CAPABILITY_MILLISECONDS}"
        ));
      }
    }
    Ok(())
  ...
  pub(crate) const fn upload_capability_lifetime(&self) -> std::time::Duration {
    std::time::Duration::from_millis(self.upload_capability_lifetime_milliseconds)
  }

  pub(crate) const fn download_capability_lifetime(&self) -> std::time::Duration {
    std::time::Duration::from_millis(self.download_capability_lifetime_milliseconds)
  }
}
...
impl CacheConfig {
  pub(crate) fn validate(&self) -> Result<(), String> {
    validate_path("cache.credential_key_file", &self.credential_key_file)?;
    octacity_server_application::validate_cache_endpoint(&self.endpoint)
      .map_err(|_| "cache.endpoint must be a credential-free HTTPS URL without query or fragment".to_owned())?;
    if self.session_lifetime_milliseconds == 0 || self.session_lifetime_milliseconds > MAX_CACHE_SESSION_MILLISECONDS {
      return Err(format!(
...
const fn default_upload_capability_milliseconds() -> u64 {
  5 * 60 * 1_000
}
