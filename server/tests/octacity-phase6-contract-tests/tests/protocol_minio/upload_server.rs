...
use octacity_artifact_store::{ArtifactId, ArtifactObject, ArtifactStore, ArtifactUploadId};
...
async fn begin_upload(
  ...
  let record = {
      ...
      .cloned()
    {
      ...
      let object = ArtifactObject::new(
        ...
        request.output.size_bytes,
        request.output.sha256.clone(),
        request.output.transport_content_type.clone(),
      )
      .map_err(|_| rejection(StatusCode::BAD_REQUEST, &request.request_id, "invalid_request"))?;
      ...
      records
        .by_key
        .insert(request.upload_key.clone(), record.object.upload_id().to_string());
      records
        .by_upload_id
        .insert(record.object.upload_id().to_string(), record.clone());
      record
    }
  };
  ...
  Ok(Json(BeginOutputUploadResponse {
    ...
    upload_id: record.object.upload_id().to_string(),
