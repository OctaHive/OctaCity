...
fn bounded_message(message: &[u8]) -> Result<(), ProtocolError> {
  adapter_core::require_message_size(message, MAX_AGENT_PROVISIONING_MESSAGE_BYTES)
    .map_err(|_| ProtocolError::LimitExceeded("encoded message"))
}

fn identifier(field: &'static str, value: &str) -> Result<(), ProtocolError> {
  adapter_core::require_bounded_text(value, MAX_IDENTIFIER_BYTES).map_err(|failure| match failure {
