//! In-memory Build cancellation and retry persistence.
...
pub(super) async fn cancel(store: &InMemoryStore, request: CancelBuild) -> Result<CancellationDisposition, StoreError> {
  ...
  for transition in decision.transitions() {
    ...
    if transition.state() == JobState::Cancelling
      && let Some(lease_id) = state.current_lease_by_job.get(&transition.job_id()).copied()
    {
      state
        .lease_states
        .insert(lease_id, octacity_server_scheduler::LeaseState::CancellationRequested);
    }
