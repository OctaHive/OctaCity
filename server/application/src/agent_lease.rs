use octacity_protocol::{AgentCredentialToken, LeaseFence as ProtocolLeaseFence};
use octacity_server_domain::{AttemptNumber, JobId, LeaseId, Timestamp};
use octacity_server_store::{LeaseAccess, LeaseFence};

use crate::{AgentOperation, AgentRegistrationError, AgentRegistrationUseCases, AuthorizeAgentInput, AuthorizedAgent};
...
pub(crate) struct AuthorizedLeaseOperation {
  pub(crate) observed_at: Timestamp,
  pub(crate) lease: ParsedLease,
}
...
pub(crate) async fn authorize_operation(
  registrations: &dyn AgentRegistrationUseCases,
...
) -> Result<AuthorizedLeaseOperation, AuthorizationError> {
  if lease.lease_id != route_lease_id {
    return Err(AuthorizationError::InvalidRequest);
  }
  let observed_at = timestamp(observed_at_unix_ms).map_err(|()| AuthorizationError::InvalidRequest)?;
  let authorized = registrations
    ...
    .await
    .map_err(AuthorizationError::Registration)?;
  let lease = parse_lease(lease, &authorized).map_err(|()| AuthorizationError::InvalidRequest)?;
  Ok(AuthorizedLeaseOperation { observed_at, lease })
}
