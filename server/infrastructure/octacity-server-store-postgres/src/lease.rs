...
pub(crate) async fn load(
  ...
  let row: LeaseRow = sqlx::query_as(
    "SELECT lease.job_id, attempt.build_id, build.project_id, job.attempt_id, attempt.attempt_number, \
            registration.id AS registration_id, agent.id AS agent_id, lease.registration_epoch AS epoch, \
