...
- [ ] 7.2 Implement pending, verifying, published, expired, and deleted artifact records tied to Job, current fence, logical name, digest, size, type, and retention policy through Rust-owned immutable-identity and state-transition decisions; verify state transitions, mutation attempts, and uniqueness constraints.
...
- [ ] 8.1 Implement append-only structured audit-fact operations for management, agent, Trigger, Orchestrator, adapter, and worker mutations without generic update or delete access; verify mutation attempts are absent from the Rust port, unauthenticated management actors are represented honestly, and sensitive request bodies are never stored.
