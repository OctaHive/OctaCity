...
- [x] 2.5 Implement concurrent adapter tests for the production operations delivered in 2.4 and separate database-level race tests for schema primitives whose application operations arrive in later tasks; verify Trigger acceptance, DAG transition, ready-Job claim, event append, completion, audit, outbox, and idempotent replay cannot expose duplicates, while schedule and retry primitives retain their database constraints until tasks 6.1 and 3.9 add their production operations.
