...
Each inter-Build Trigger chooses one downstream source strategy. `inherit revision` reuses the upstream Build's already-resolved immutable revision and is valid only when the two configuration snapshots name the same compatible Repository identity. `resolve target source` uses a bounded source expression allowed by the downstream configuration and resolves it independently. The trigger definition contains bounded constant parameters only; interpreting upstream logs, reports, artifact contents, or arbitrary expressions as downstream parameters is outside the initial contract.

One internal Trigger models one upstream event to one downstream Build. Multiple definitions provide fan-out, and causally linked definitions provide chains such as `A -> B -> C`. Waiting for several independent Builds before starting another Build is not inter-Build fan-in in the initial release; when the work shares one execution lifecycle, its dependency belongs in a Pipeline DAG. Direct definition validation rejects self-dependencies and incompatible revision inheritance, while durable ancestry and depth protection remain authoritative for cycles formed across versions or concurrent changes.
...
Automatic retention deadlines for Build metadata, archived logs, artifacts and reports are materialized from the Build's immutable effective-policy snapshot rather than recalculated from the current Project policy. A **Build Result retention hold**, presented to operators as a pin, is a separate versioned record over the whole Build Result rather than a mutation of those deadlines or an individual object. It records a bounded reason, creation time, available management actor and request identity, optional expiry, and release state. Build-level scope keeps the configuration snapshot, history, logs and outputs coherent; the initial release does not offer component-specific pins. Permanently or temporarily held bytes continue to count toward the applicable quota.

Retention eligibility is derived as `deadline elapsed AND no active Build Result hold AND no other retained reference`. Placing or releasing a hold and the first transition that hides a Build Result serialize through one authoritative store operation. Therefore either the hold wins before any component becomes invisible, or the command receives a stable conflict after deletion starts; it cannot revive tombstoned metadata or missing bytes. Releasing or expiring the last hold does not move the original deadlines, so already-overdue components are claimable immediately. The ordinary idempotency and audit transaction rules apply to hold commands.
...
Due schedules, trigger evaluation, DAG transitions, lease expiry, webhook/VCS retries, outbox delivery, build-log indexing and rebuild, Build Result retention, pending-artifact and orphan-log cleanup, cache retention, and audit retention are store-backed work. Each worker claims bounded batches with owner and deadline, performs idempotent work, rechecks active holds and retained references before the first visibility transition, and records completion.
